...
def extract_weights_with_context(text, target_units):
    ...
    if matches:
        ...
        for match in matches:
            ...
            if weight_unit.lower() in target_units or "all" in target_units:
...
def detect_and_print_weights():
    ...
    # Récupérer l'unité sélectionnée par l'utilisateur
    selected_unit = unit_choice.get()

    # Déterminer les unités cibles en fonction de la sélection de l'utilisateur
    if selected_unit == "all":
        target_units = ["kg", "g", "lbs"]
    else:
...
def main():
    ...
    # Options d'unité disponibles
    unit_choice = StringVar(root)
    unit_choice.set("all")  # Option par défaut "Toutes"

    unit_label = tk.Label(root, text="Choisir l'unité à rechercher :")
    unit_label.pack(pady=5)

    unit_menu = tk.OptionMenu(root, unit_choice, "all", "kg", "g", "lbs")
